...
import { fetchWithTimeout } from "./fetch-utils-C_Xj921x.js";
import { z } from "zod";
import { setTag } from "@sentry/core";

//#region src/tools/get-doc.ts
var get_doc_default = defineTool({
	name: "get_doc",
	description: [
	...
	inputSchema: { path: z.string().trim().describe("The documentation path (e.g., '/platforms/javascript/guides/nextjs.md'). Get this from search_docs results.") },
	async handler(params, context) {
		setTag("doc.path", params.path);
		let output = `# Documentation Content\n\n`;
		output += `**Path**: ${params.path}\n\n`;
		if (!params.path.endsWith(".md")) throw new UserInputError("Invalid documentation path. Path must end with .md extension.");
		...
		if (!allowedDomains.includes(docUrl.hostname)) throw new UserInputError(`Invalid domain. Documentation can only be fetched from allowed domains: ${allowedDomains.join(", ")}`);
		const response = await fetchWithTimeout(docUrl.toString(), { headers: {
		...
		if (!response.ok) {
			...
			throw new ApiError(`Failed to fetch documentation: ${response.statusText}`, response.status);
		}
...
//#endregion
export { get_doc_default };
//# sourceMappingURL=get-doc-DyqJe6TP.js.map