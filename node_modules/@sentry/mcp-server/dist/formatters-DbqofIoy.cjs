...
function formatIssueResults(issues, organizationSlug, projectSlugOrId, query, regionUrl, naturalLanguageQuery, skipHeader = false) {
	...
	if (issues.length === 0) {
		__sentry_node.logger.info(__sentry_node.logger.fmt`No issues found for query: ${naturalLanguageQuery || query}`, {
			query,
			organizationSlug,
	...
	const searchUrl = require_url_utils.getIssuesSearchUrl(host, organizationSlug, query, projectSlugOrId);
	...
	issues.forEach((issue, index) => {
		const issueUrl = require_url_utils.getIssueUrl(host, organizationSlug, issue.shortId);
...
//#endregion
Object.defineProperty(exports, 'formatIssueResults', {
  enumerable: true,
  get: function () {
    return formatIssueResults;
  }
});
//# sourceMappingURL=formatters-DbqofIoy.cjs.map