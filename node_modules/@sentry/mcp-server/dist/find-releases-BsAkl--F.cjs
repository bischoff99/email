...
//#region src/tools/find-releases.ts
var find_releases_default = require_defineTool.defineTool({
	name: "find_releases",
	description: [
	...
	inputSchema: {
		...
		projectSlug: require_schema.ParamProjectSlugOrAll.optional(),
		query: zod.z.string().trim().describe("Search for versions which contain the provided string.").optional()
	},
	async handler(params, context) {
		...
		output += releases.map((release) => {
			const releaseInfo = [
				`## ${release.shortVersion}`,
				"",
				`**Created**: ${new Date(release.dateCreated).toISOString()}`
			];
			if (release.dateReleased) releaseInfo.push(`**Released**: ${new Date(release.dateReleased).toISOString()}`);
			if (release.firstEvent) releaseInfo.push(`**First Event**: ${new Date(release.firstEvent).toISOString()}`);
			if (release.lastEvent) releaseInfo.push(`**Last Event**: ${new Date(release.lastEvent).toISOString()}`);
			if (release.newGroups !== void 0) releaseInfo.push(`**New Issues**: ${release.newGroups}`);
			if (release.projects && release.projects.length > 0) releaseInfo.push(`**Projects**: ${release.projects.map((p) => p.name).join(", ")}`);
			if (release.lastCommit) {
				...
				releaseInfo.push(`**Commit Author**: ${release.lastCommit.author.name}`);
				releaseInfo.push(`**Commit Date**: ${new Date(release.lastCommit.dateCreated).toISOString()}`);
			}
			if (release.lastDeploy) {
				releaseInfo.push("", `### Last Deploy`, "");
				releaseInfo.push(`**Deploy ID**: ${release.lastDeploy.id}`);
				releaseInfo.push(`**Environment**: ${release.lastDeploy.environment}`);
				if (release.lastDeploy.dateStarted) releaseInfo.push(`**Deploy Started**: ${new Date(release.lastDeploy.dateStarted).toISOString()}`);
				if (release.lastDeploy.dateFinished) releaseInfo.push(`**Deploy Finished**: ${new Date(release.lastDeploy.dateFinished).toISOString()}`);
			}
...
//#endregion
Object.defineProperty(exports, 'find_releases_default', {
  enumerable: true,
  get: function () {
    return find_releases_default;
  }
});
//# sourceMappingURL=find-releases-BsAkl--F.cjs.map