...
async function fetchCustomAttributes(apiService, organizationSlug, dataset, projectId, timeParams) {
	...
	try {
		...
		require_logging.logError(error, { search_events: {
			dataset,
			organizationSlug,
...
function createDatasetAttributesTool(apiService, organizationSlug, projectId) {
	return (0, ai.tool)({
		...
		parameters: zod.z.object({ dataset: zod.z.enum([
			"spans",
			"errors",
			"logs"
		...
		execute: async ({ dataset }) => {
			try {
				const { BASE_COMMON_FIELDS, DATASET_FIELDS, RECOMMENDED_FIELDS, NUMERIC_FIELDS } = await Promise.resolve().then(() => require("./tools/search-events/config.cjs"));
...
//#endregion
Object.defineProperty(exports, 'createDatasetAttributesTool', {
  enumerable: true,
  get: function () {
    return createDatasetAttributesTool;
  }
});
Object.defineProperty(exports, 'fetchCustomAttributes', {
  enumerable: true,
  get: function () {
...
Object.defineProperty(exports, 'isAggregateQuery', {
  enumerable: true,
  get: function () {
    return isAggregateQuery;
  }
});
//# sourceMappingURL=utils-CqbeJfE1.cjs.map