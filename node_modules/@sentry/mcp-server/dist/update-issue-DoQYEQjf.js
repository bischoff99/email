...
import { ParamAssignedTo, ParamIssueShortId, ParamIssueStatus, ParamIssueUrl, ParamOrganizationSlug, ParamRegionUrl } from "./schema-DMfkTR1l.js";
import { parseIssueParams } from "./issue-helpers-CRx45FYG.js";
import { formatAssignedTo } from "./formatting-utils-C3sej5ER.js";
import { setTag } from "@sentry/core";

//#region src/tools/update-issue.ts
var update_issue_default = defineTool({
	name: "update_issue",
	description: [
	...
	async handler(params, context) {
		...
		if (!params.status && !params.assignedTo) throw new UserInputError("At least one of `status` or `assignedTo` must be provided to update the issue");
		...
		const currentIssue = await withApiErrorHandling(() => apiService.getIssue({
		...
		const updatedIssue = await withApiErrorHandling(() => apiService.updateIssue({
			organizationSlug: orgSlug,
			issueId: parsedIssueId,
...
//#endregion
export { update_issue_default };
//# sourceMappingURL=update-issue-DoQYEQjf.js.map