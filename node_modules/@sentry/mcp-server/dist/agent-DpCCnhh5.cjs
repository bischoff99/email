...
const require_lookup_otel_semantics = require('./lookup-otel-semantics-BbriXZAx.cjs');
...
//#region src/tools/search-events/agent.ts
/**
* Translate natural language query to Sentry search syntax using AI
*/
async function translateQuery(params, apiService, organizationSlug, projectId) {
	if (!process.env.OPENAI_API_KEY) throw new require_errors.ConfigurationError("OPENAI_API_KEY environment variable is required for semantic search");
...
- For time periods: Use timeRange parameter (NOT SQL date functions like yesterday())`;
	const datasetAttributesTool = require_utils.createDatasetAttributesTool(apiService, organizationSlug, projectId);
	const otelLookupTool = require_lookup_otel_semantics.createOtelLookupTool(apiService, organizationSlug, projectId);
	const whoamiTool = require_whoami.createWhoamiTool(apiService);
	const result = await (0, ai.generateText)({
		model: (0, __ai_sdk_openai.openai)("gpt-4o"),
		system: systemPrompt,
		...
		maxSteps: 5,
		temperature: .1,
		experimental_output: ai.Output.object({ schema: zod.z.object({
			dataset: zod.z.enum([
				"spans",
				"errors",
				"logs"
			]).optional().describe("Which dataset to use for the query"),
			query: zod.z.string().optional().describe("The Sentry query string for filtering results (empty string returns all recent events)"),
			fields: zod.z.array(zod.z.string()).optional().describe("Array of field names to return in results. REQUIRED for aggregate queries (include only aggregate functions and groupBy fields). Optional for individual event queries (will use recommended fields if not provided)."),
			sort: zod.z.string().optional().describe("REQUIRED: Sort parameter for results (e.g., '-timestamp' for newest first, '-count()' for highest count first)"),
			timeRange: zod.z.union([zod.z.object({ statsPeriod: zod.z.string().describe("Relative time period like '1h', '24h', '7d'") }), zod.z.object({
				start: zod.z.string().describe("ISO 8601 start time"),
				end: zod.z.string().describe("ISO 8601 end time")
			})]).optional().describe("Time range for filtering events. Use either statsPeriod for relative time or start/end for absolute time."),
			error: zod.z.string().optional().describe("Error message if the query cannot be translated")
		}).superRefine((data, ctx) => {
			if (!data.error) {
				if (data.dataset === void 0) ctx.addIssue({
					code: zod.z.ZodIssueCode.custom,
					message: "Dataset is required when no error is returned.",
					path: ["dataset"]
				});
				if (data.sort === void 0) ctx.addIssue({
					code: zod.z.ZodIssueCode.custom,
	...
	try {
		...
		return { error: `AI failed to generate a valid query translation. This may happen when the query is too complex or ambiguous. Please try rephrasing your query. Details: ${errorMessage}` };
	}
	return {
...
//# sourceMappingURL=agent-DpCCnhh5.cjs.map