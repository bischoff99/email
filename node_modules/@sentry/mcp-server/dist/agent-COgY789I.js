...
import { createOtelLookupTool } from "./lookup-otel-semantics-Dt667kdR.js";
import { createWhoamiTool } from "./whoami-xBAWvzG4.js";
import { createDatasetAttributesTool } from "./utils-OgYE-imr.js";
import { z } from "zod";
...
async function translateQuery(params, apiService, organizationSlug, projectId) {
	if (!process.env.OPENAI_API_KEY) throw new ConfigurationError("OPENAI_API_KEY environment variable is required for semantic search");
...
- For time periods: Use timeRange parameter (NOT SQL date functions like yesterday())`;
	const datasetAttributesTool = createDatasetAttributesTool(apiService, organizationSlug, projectId);
	const otelLookupTool = createOtelLookupTool(apiService, organizationSlug, projectId);
	...
	const result = await generateText({
		...
		experimental_output: Output.object({ schema: z.object({
			...
			query: z.string().optional().describe("The Sentry query string for filtering results (empty string returns all recent events)"),
			fields: z.array(z.string()).optional().describe("Array of field names to return in results. REQUIRED for aggregate queries (include only aggregate functions and groupBy fields). Optional for individual event queries (will use recommended fields if not provided)."),
			sort: z.string().optional().describe("REQUIRED: Sort parameter for results (e.g., '-timestamp' for newest first, '-count()' for highest count first)"),
			timeRange: z.union([z.object({ statsPeriod: z.string().describe("Relative time period like '1h', '24h', '7d'") }), z.object({
			...
			error: z.string().optional().describe("Error message if the query cannot be translated")
...
//# sourceMappingURL=agent-COgY789I.js.map