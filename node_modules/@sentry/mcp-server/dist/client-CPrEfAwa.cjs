...
var SentryApiService = class {
	...
	*/
	isSaas() {
		return require_url_utils.isSentryHost(this.host);
	}
	/**
	...
	*/
	async request(path, options = {}, { host } = {}) {
		...
		if (typeof globalThis.fetch === "undefined") throw new require_errors.ConfigurationError("fetch is not available. Please use Node.js >= 18 or ensure fetch is available in your environment.");
		let response;
		try {
			...
			if (errorCode === "ENOTFOUND" || errorCode === "EAI_AGAIN" || errorCode === "ECONNREFUSED" || errorCode === "ETIMEDOUT" || errorMessage.includes("Connect Timeout Error")) throw new require_errors.ConfigurationError(friendlyMessage, { cause: error });
		...
		if (!response.ok) {
			...
			if (parsed) {
				const { data, success, error } = require_schema.ApiErrorSchema.safeParse(parsed);
	...
	*/
	getIssueUrl(organizationSlug, issueId) {
		return require_url_utils.getIssueUrl(this.host, organizationSlug, issueId);
	}
	/**
	...
	*/
	getTraceUrl(organizationSlug, traceId) {
		return require_url_utils.getTraceUrl(this.host, organizationSlug, traceId);
	}
	/**
	...
	*/
	async getAuthenticatedUser(opts) {
		...
		return require_schema.UserSchema.parse(body);
	}
	/**
	...
	*/
	async listOrganizations(opts) {
		if (!this.isSaas()) {
			...
			return require_schema.OrganizationListSchema.parse(body);
		}
		try {
			const regionsBody = await this.requestJSON("/users/me/regions/", void 0, {});
			const regionData = require_schema.UserRegionsSchema.parse(regionsBody);
			...
			})))).map((data) => require_schema.OrganizationListSchema.parse(data)).reduce((acc, curr) => acc.concat(curr), []);
		} catch (error) {
	...
	*/
	async listTeams(organizationSlug, opts) {
		...
		return require_schema.TeamListSchema.parse(body);
	}
	/**
	...
	*/
	async createTeam({ organizationSlug, name }, opts) {
		...
		return require_schema.TeamSchema.parse(body);
	}
	/**
	...
	*/
	async listProjects(organizationSlug, opts) {
		...
		return require_schema.ProjectListSchema.parse(body);
	}
	/**
	...
	*/
	async getProject({ organizationSlug, projectSlugOrId }, opts) {
		...
		return require_schema.ProjectSchema.parse(body);
	}
	/**
	...
	*/
	async createClientKey({ organizationSlug, projectSlug, name }, opts) {
		...
		return require_schema.ClientKeySchema.parse(body);
	}
	/**
	...
	*/
	async listClientKeys({ organizationSlug, projectSlug }, opts) {
		...
		return require_schema.ClientKeyListSchema.parse(body);
	}
	/**
	...
	*/
	async listReleases({ organizationSlug, projectSlug, query }, opts) {
		...
		return require_schema.ReleaseListSchema.parse(body);
	}
	/**
	...
	*/
	async listTags({ organizationSlug, dataset, project, statsPeriod, start, end, useCache, useFlagsBackend }, opts) {
		...
		return require_schema.TagListSchema.parse(body);
	}
	/**
	...
	*/
	async listIssues({ organizationSlug, projectSlug, query, sortBy, limit = 10 }, opts) {
		...
		return require_schema.IssueListSchema.parse(body);
	}
	async getIssue({ organizationSlug, issueId }, opts) {
		...
		return require_schema.IssueSchema.parse(body);
	}
	async getEventForIssue({ organizationSlug, issueId, eventId }, opts) {
		...
		return require_schema.EventSchema.parse(body);
	}
	...
	async listEventAttachments({ organizationSlug, projectSlug, eventId }, opts) {
		...
		return require_schema.EventAttachmentListSchema.parse(body);
	}
	async getEventAttachment({ organizationSlug, projectSlug, eventId, attachmentId }, opts) {
		...
		const attachments = require_schema.EventAttachmentListSchema.parse(attachmentsData);
	...
	async searchErrors({ organizationSlug, projectSlug, filename, transaction, query, sortBy = "last_seen" }, opts) {
		...
		const body = await this.requestJSON(apiUrl, void 0, opts);
		return require_schema.ErrorsSearchResponseSchema.parse(body).data;
	}
	async searchSpans({ organizationSlug, projectSlug, transaction, query, sortBy = "timestamp" }, opts) {
		...
		const apiUrl = `/organizations/${organizationSlug}/events/?${queryParams.toString()}`;
		const body = await this.requestJSON(apiUrl, void 0, opts);
		return require_schema.SpansSearchResponseSchema.parse(body).data;
	}
	/**
	...
	*/
	...
	async startAutofix({ organizationSlug, issueId, eventId, instruction = "" }, opts) {
		...
		return require_schema.AutofixRunSchema.parse(body);
	}
	async getAutofixState({ organizationSlug, issueId }, opts) {
		...
		return require_schema.AutofixRunStateSchema.parse(body);
	}
};
...
Object.defineProperty(exports, 'SentryApiService', {
  enumerable: true,
  get: function () {
    return SentryApiService;
  }
});
//# sourceMappingURL=client-CPrEfAwa.cjs.map