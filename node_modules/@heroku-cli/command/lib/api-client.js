    ...
    constructor(config, options = {}) {
        ...
        this.http = class APIHTTPClient extends deps_1.default.HTTP.HTTP.create(opts) {
            ...
            static trackRequestIds(response) {
                const responseRequestIdHeader = response.headers[request_id_1.requestIdHeader] || response.headers[request_id_1.requestIdHeader.toLocaleLowerCase()];
            ...
            static showWarnings(response) {
                ...
                if (Array.isArray(warnings))
                    warnings.forEach(warning => (0, errors_1.warn)(`${warning}\n`));
                else if (typeof warnings === 'string')
                    (0, errors_1.warn)(`${warnings}\n`);
            }
            ...
            static async request(url, opts = {}, retries = 3) {
                opts.headers = opts.headers || {};
                const currentRequestId = request_id_1.RequestId.create() && request_id_1.RequestId.headerValue;
                ...
                if (Buffer.from(currentRequestId).byteLength > 1024 * 7) {
                    request_id_1.RequestId.empty();
                    opts.headers[request_id_1.requestIdHeader] = request_id_1.RequestId.create();
                }
                else {
                    opts.headers[request_id_1.requestIdHeader] = currentRequestId;
                }
